...
def_HttpEntry(SqlRun, req){
    ...
    int count;
    string res;
    int errCode = PRELOAD_DB.Query(sql, count, res);
...
def_HttpEntry(SqlTest, req){
    ...
    int errCode = PRELOAD_DB.Query("drop table database_test;", count, res);
    if(errCode != NO_ERROR){
        CONSOLE_LOG(0, 1, 1, "Warning : Query %d OK return Code %d\n", id, errCode);
        return new HttpResponse{std::to_string(errCode)};
    }
    errCode = PRELOAD_DB.Query("create table database_test(id int,name text);", count, res);
    if(errCode != NO_ERROR){
        CONSOLE_LOG(0, 1, 1, "Warning : Query %d OK return Code %d\n", id, errCode);
        return new HttpResponse{std::to_string(errCode)};
    }
    errCode = PRELOAD_DB.Query(DataBaseTestSQL.c_str(), count, res);
    CONSOLE_LOG(0, 1, 1, "Warning : Query %d OK return Code %d\n", id, errCode);
    return new HttpResponse{std::to_string(errCode)};
