    ...
    */
    ...
    if(!page->is_full_){
        if(!page->InsertRow(new_row)){
            cout<<"<E> PRIM KEY VALUE '"<<new_row->getIndex()<<"' EXISTS."<<endl;
            return false;
        }
    }else{
    //////新建页后插入/////////////////////////////////////////////////
...
bool Page::InsertRow(Row* Node){
    for(__uint16_t i = 0; i < cursor_pos_; i++){
        if(rows_[i]->getIndex() == Node->getIndex()) return false;
    }
    rows_[cursor_pos_ ++] = Node;
    q_sort(rows_,0,cursor_pos_-1);
    cout<<"C/M : "<<cursor_pos_<<" / "<<table_ptr_->max_rows_per_page_<<endl;
    if(cursor_pos_ >= table_ptr_->max_rows_per_page_) is_full_ = true;
    return true;
}
...
bool Row::Padding(string condition,string value){
    ...
    */
    ...
    //判断元素个数匹配
    if( cond_num != value_num 
        || cond_num > table_ptr_->parm_num_ 
    ...
    //主键检测;
    if(condition.find(table_ptr_->parm_names_[table_ptr_->prim_key_]) == condition.npos){
        cout<<"<E> PRIM KEY VALUE REQUIRED"<<endl;
        return false;
    }
    //定位填充
    for(int i = 0; i < value_num ; i++){
        int pos = table_ptr_->parameter_locate(conds[i]);
        if(pos == -1){
            cout<<"<E> PARAMETER NOT FOUND"<<endl;
            return false;
        }
        if(!value_update(pos,values[i]))    return false;
    }
    index_update();
    return true;
}  
...
bool Row::value_update(int pos,string value){
    ...
    if(!parm_check(value,type)) return false;
    switch(type){
        case __INT :
            try{
